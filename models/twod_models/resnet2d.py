...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.tam is not None:
            ...
            x = x.view(batch_size * self.num_frames, c_t // self.num_frames, h, w)
            x = self.tam(x)
            x = x.view(batch_size, c_t, h, w)
...
#        x = x.view(batch_size * self.orig_num_frames, c_t // self.orig_num_frames, h, w)
        ...
        batch_size, c_t, h, w = fp5.shape
        fp5 = fp5.view(batch_size * self.orig_num_frames, c_t//self.orig_num_frames, h, w)
        x = self.avgpool(fp5)
        x = x.view(x.size(0), -1)
        x = self.dropout(x)
        x = self.fc(x)
    ...
    def network_name(self):
        ...
        name += 'resnet2d-{}'.format(self.depth)
        if not self.without_t_stride:
...
def resnet2d(depth, num_classes, without_t_stride, groups, temporal_module_name,
    ...
    model = ResNet2D(depth, num_frames=groups, num_classes=num_classes,
...
if __name__ == '__main__':
    ...
    model = resnet(50, 187, without_t_stride=False, groups=8, dropout=0.5,
