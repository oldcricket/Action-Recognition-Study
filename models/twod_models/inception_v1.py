...
def make_non_local(net, num_frames):
    from models.twod_models.ops.non_local import NL3DWrapperV2
    net.inception3b = nn.Sequential(
        net.inception3b,
        NL3DWrapperV2(net.inception3b.out_channels, num_frames)
    )
    net.inception4b = nn.Sequential(
        net.inception4b,
        NL3DWrapperV2(net.inception4b.out_channels, num_frames)
    )
    net.inception5b = nn.Sequential(
        net.inception5b,
        NL3DWrapperV2(net.inception5b.out_channels, num_frames)
    )
...
def inception_v1(num_classes, without_t_stride, groups, dropout, temporal_module_name,
    ...
    if add_non_local:
        model = make_non_local(model, groups)
...
if __name__ == '__main__':
    ...
    model = inception_v1(187, without_t_stride=True, groups=8, dropout=0.5,
                         temporal_module_name='NonLocal', dw_conv=True, blending_frames=3,
                         blending_method='sum', pooling_method='max')
