...
class TAM3D(nn.Module):
    ...
    def __init__(self, channels, blending_frames=3):
        super().__init__()
...
class STBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=(1, 1, 1), padding=0, downsample=None):
        ...
        self.tam = TAM3D(inplanes, blending_frames=3)

        self.conv1 = BasicConv3d(inplanes, planes, kernel_size=(1, 3, 3),
                                 stride=(stride[0], stride[1], stride[2]), padding=(0, padding, padding),
                                 bias=False)
...
class S3D_ResNet(nn.Module):
    def __init__(self, depth, num_classes=1000, dropout=0.5, without_t_stride=False,
        ...
        self.conv1 = BasicConv3d(3, 64, kernel_size=(1, 7, 7), stride=(1, 2, 2), padding=(0, 3, 3),
                                 bias=False)
    ...
    def network_name(self):
        name = 's3d-resnet-tam-{}'.format(self.depth) if self.without_t_stride \
            else 's3d-resnet-tam-{}-ts'.format(self.depth)
        return name
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, stride=(self.t_s if stride == 2 else 1, stride, stride),
...
def s3d_resnet_tam(depth, num_classes, dropout, without_t_stride, **kwargs):
...
if __name__ == '__main__':
    ...
    model = s3d_resnet_tam(50, 400, 0.5, without_t_stride=True)

    dummy_data = (3, 8, 224, 224)
    model.eval()
