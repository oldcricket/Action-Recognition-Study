...
def main_worker(gpu, ngpus_per_node, args):
    ...
    val_augmentor = get_augmentor(False, args.input_size, scale_range=args.scale_range, mean=mean, std=std, disable_scaleup=args.disable_scaleup,
                                  threed_data=args.threed_data, is_flow=True if args.modality == 'flow' else False,
                                  version=args.augmentor_ver)

    video_data_cls = VideoDataSetLMDB if args.use_lmdb else VideoDataSet
    val_dataset = video_data_cls(args.datadir, val_list, args.groups, args.frames_per_group, num_clips=args.num_clips,
    ...
    train_augmentor = get_augmentor(True, args.input_size, scale_range=args.scale_range, mean=mean, std=std,
                                    ...
                                    is_flow=True if args.modality == 'flow' else False, version=args.augmentor_ver)

    train_dataset = video_data_cls(args.datadir, train_list, args.groups, args.frames_per_group, num_clips=args.num_clips,
                                   modality=args.modality, image_tmpl=image_tmpl, dense_sampling=args.dense_sampling,
